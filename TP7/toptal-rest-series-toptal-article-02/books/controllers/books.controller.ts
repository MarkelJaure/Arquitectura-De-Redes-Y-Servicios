...
class BooksController {
  ...
  async getBookByUserAndId(req: express.Request, res: express.Response) {
    const book = await booksService.readById(req.body.id);
    res.status(200).send(book);
  }

  async getBooksByUserId(req: express.Request, res: express.Response) {
    const booksByUser = await booksService.getBooksByUser(req.body.userId);
    res.status(200).send(booksByUser);
  }
}
